...
int main() 
{
	const char *addr1 = "10.0.1.2";
	const char *addr2 = "192.168.0.2";
	const char *gate1 = "172.0.0.2";
	const char *gate2 = "0.0.0.0";
	...
	while(true) 
	{
		...
		if(sall.sll_protocol == htons(ETH_P_IP)) {
			if(hip->daddr == inet_addr(my_ip)) {//发到本路由器
				if(hip->protocol == IPPROTO_IPIP) {
					print_ip(hip->saddr);
					print_ip(hip->daddr);
					
					hip = (struct iphdr*)(temp_buf + hip->ihl * 4);
